...
> - `CONCEPTS` (~110 entries) are matched by **word-boundary keyword search against the card's own text**, which splits the mega-buckets. **Term-length rule:** terms of ≤4 chars are treated as abbreviations and get a hard end boundary (plus optional plural s); longer terms are deliberate stems and stay open-ended so `neonat` catches neonatal/neonate and `vaccin` catches vaccine/vaccination. Without that split, `abi` matched "ability", `ips` matched "ipsilateral", and `eps` matched "epsilon". Also: **don't add a short abbreviation that's an ordinary English word** — a bare `last` for Local Anesthetic Systemic Toxicity matched 14 cards, every one of them "how long does it last" / "headache lasts 4-72h". Deterministic, offline, no API cost — important, because AI-classifying 1,929 cards would spend the user's money on a build-time job.
