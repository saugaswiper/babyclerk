...
| `babyclerk:attempts` | per-answer log `[{id, ts, rotation, kind, cardId, topic, concept, conceptId, correct, grade, latencyMs}]` (capped 4000). `concept`/`conceptId` are resolved from the card at **write** time via `lib/ontology.js`; records written before the ontology existed have neither and fall back to `topic` (`topicKeyOf()`) | ✅ union by id, capped |
